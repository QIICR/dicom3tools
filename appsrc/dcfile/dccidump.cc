	...
	while (!listi) {
		...
		if (strcmp(a->getVR(),"SQ") == 0) {
			...
			if ((n=a->getLists(&al)) > 0) {
				if (n>0 && (*al[0])[TagFromName(ValueType)]) {
					indent(log,depth,true,false/*showgt*/);
				...
				for (i=0; i<n; ++i) {
					...
					success&=processContentItem(al[i],log,verbose,veryverbose,depth+1,childidentifierstring);
...
static bool
parseContentItems(ManagedAttributeList &list,TextOutputStream &log,bool verbose,bool veryverbose,bool showidentifier)
{
...
main(int argc, char *argv[])
{
	...
	{
		bool parseSuccess = parseContentItems(list,log,verbose,veryverbose,showidentifier);		// parse regardless of read success or failure
