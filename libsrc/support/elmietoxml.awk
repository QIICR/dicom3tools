#  elmietoxml.awk Copyright (c) 1993-2024, David A. Clunie DBA PixelMed Publishing. All rights reserved.
NR==1	{
	print "<?xml version=\"1.0\" encoding=\"UTF-8\" ?>"
	print "<!DOCTYPE DataDictionaryIE ["
	print "<!ELEMENT DataDictionaryIE (DataElementIE*)>"
	print "<!ELEMENT DataElementIE (Name,IE)>"
	print "<!ELEMENT Name (#PCDATA)>"
	print "<!ELEMENT IE (#PCDATA)>"
	print "]>"
	print ""
	print "<DataDictionaryIE xmlns:Dicom=\"http://www.nema.org/medical/dicom\">"
...
/^[ 	]*(Name|Sequence)=/ {
	...
	if (name != "" && (module != "" || macroie != "")) {
		...
		if (!(name in predefinedIEByName)) {		# else will be written out in bulk at the end
			...
			else if (ourie == "Acquisition") useie="INSTANCE"
			else if (ourie == "MultiResolutionPyramid") useie="INSTANCE"
			...
			else if (ourie == "Annotation") useie="INSTANCE"
			else if (ourie == "Frame") {
				if (emitFrameLevel) {
					useie="FRAME"
				}
			...
			if (useie != "") {
				...
				ieByName[name]=useie;
				print "\t<DataElementIE><Name>" name "</Name><IE>" useie "</IE></DataElementIE>"
			}
		}
	}
}
